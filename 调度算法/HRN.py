            ...
            if process_list != [] and process_list[0].arrive_time <= running_time:
                ...
                if buffer_list != []:
                    for i in buffer_list:
                        i.response_ratio = (running_time - i.arrive_time) / i.serve_time + 1
                        #print(i.response_ratio)

                    buffer_list.sort(key = lambda x: x.response_ratio, reverse = True)  # 按照响应比降序排序

                    buffer_list[0].start_serve_time = running_time  # 开始时间
                    running_time += buffer_list[0].serve_time
                    buffer_list[0].finish_time = running_time  # 结束时间
                    buffer_list[0].cycling_time = buffer_list[0].finish_time - buffer_list[0].arrive_time  # 周转时间
                    buffer_list[0].w_cycling_time = buffer_list[0].cycling_time / buffer_list[0].serve_time  # 带权周转时间
                    pr.append(buffer_list[0])
                    buffer_list.pop(0)
